<template>
    <v-card outlined>
        ...
        <v-card-text style="background-color: white;">
            ...
            <RestaurantId offline label="RestaurantId" v-model="value.restaurantId" :editMode="editMode" @change="change"/>
            <MenuId offline label="MenuId" v-model="value.menuId" :editMode="editMode" @change="change"/>
            <DeliveryId offline label="DeliveryId" v-model="value.deliveryId" :editMode="editMode" @change="change"/>
        </v-card-text>
        ...
        <v-card-actions>
            ...
            <v-dialog v-model="createOrderDiagram" width="500">
                <CreateOrderCommand
                    @closeDialog="closeCreateOrder"
	...
	async created() {
        ...
        methods: {
            ...
            async createOrder() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links['/createorder'].href))
                        for(var k in temp.data) this.value[k]=temp.data[k];
                    }
